...
with patch('backend.core.supabase.supabase'):
    from backend.manage_db import get_all_users, delete_user, clear_user_games, reset_all_data, main

@pytest.fixture
def mock_supabase():
    with patch('backend.manage_db.supabase') as mock_client:
        yield mock_client
...
def test_reset_all_data_success(mock_supabase):
    ...
    with patch('backend.manage_db.get_all_users') as mock_get_users:
...
# Test the main CLI interactive flow (Menu choice 7 to exit initially)
def test_main_exit():
